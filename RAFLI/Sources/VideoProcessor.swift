...
final class VideoProcessor {
    static func export(source: URL, preset: RAFLIPreset, report: VideoReport, progress: @escaping @Sendable (Double) -> Void) async throws -> URL {
        if preset == .preserve {
            return try await passthrough(source: source)
        }
    ...
    private static func passthrough(source: URL) async throws -> URL {
        ...
        guard let session = AVAssetExportSession(asset: asset, presetName: AVAssetExportPresetPassthrough) else {
            throw err(2, "Passthrough unavailable")
        }
        ...
        guard session.status == .completed else {
            throw session.error ?? err(3, "Export failed")
        }
        return out
    }
    ...
    private static func transcode(source: URL, preset: RAFLIPreset, report: VideoReport, progress: @escaping @Sendable (Double) -> Void) async throws -> URL {
        ...
        let targetW = portrait ? 1080 : 1920
        let targetH = portrait ? 1920 : 1080
        let sourceFPS = max(1.0, report.fps)
        let targetFPS = min(60.0, sourceFPS)
        let targetBitrate: Int = preset == .smart
            ? min(22_000_000, max(10_000_000, Int(max(6.0, report.bitrateMbps) * 1_000_000)))
        ...
        guard reader.startReading(), writer.startWriting() else {
            throw reader.error ?? writer.error ?? err(7, "Could not start encoding")
        }
        ...
        group.enter()
        let vQ = DispatchQueue(label: "com.ucorc.rafli.video", qos: .userInitiated)
        vIn.requestMediaDataWhenReady(on: vQ) {
            while vIn.isReadyForMoreMediaData {
                if stateQ.sync(execute: { failure != nil }) {
                ...
                let ie = image.extent
                let scale = max(targetRect.width / ie.width, targetRect.height / ie.height)
        ...
        if let aOut, let aIn {
            group.enter()
            let aQ = DispatchQueue(label: "com.ucorc.rafli.audio", qos: .userInitiated)
            ...
            group.notify(queue: .global(qos: .userInitiated)) {
                ...
                writer.finishWriting {
                    if writer.status == .completed {
                        ...
                        cont.resume(throwing: writer.error ?? err(12, "Writer failed"))
                    }
                }
            }
        }
    ...
    private static func err(_ code: Int, _ text: String) -> NSError {
        NSError(domain: "RAFLI", code: code, userInfo: [NSLocalizedDescriptionKey: text])
    }

    private static func tempURL(_ name: String) -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString + "_" + name)
    }
}
